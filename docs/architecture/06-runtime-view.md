...
## 6.4 Scenario: Follow Relationship Update

When user A follows user B, the User Service writes the follow edge to PostgreSQL and emits `follow.created`.

**Key points:**
- PostgreSQL remains the source of truth for follow relationships.
- The Feed Service may consume `follow.created` to trigger optional feed pre-warm tasks, but correctness does not depend on pre-warm.
- If pre-warm is skipped or delayed, feed reads remain correct via SQL fallback (`follows` + `posts`).
